...
__PROJECT__ = "LYTTLE reSearch - Chromatic Vector Fusion Engine"
...
def save_opengl_screenshot(width=1024, height=768):
    ...
    """
    try:
        # Read the raw pixel block bytes directly out of GPU memory
        glPixelStorei(GL_PACK_ALIGNMENT, 1)
        data = glReadPixels(0, 0, width, height, GL_RGB, GL_UNSIGNED_BYTE)
        
        # Convert the raw byte stream into an asset surface Pygame can manipulate
        image_surface = pygame.image.fromstring(data, (width, height), 'RGB')
        
        # OpenGL maps (0,0) to the bottom-left corner; flip vertically to align to standard image layouts
        image_surface = pygame.transform.flip(image_surface, False, True)
        
        # Write image asset out safely using high-precision timestamp markers
...
def main():
    ...
    # Establish workspace title tracking branding parameters
    ...
    try:
        while running:
            ...
            if scan_data:
                glPointSize(max(2.0, min(16.0, 5.0 * zoom)))
                glBegin(GL_POINTS)
