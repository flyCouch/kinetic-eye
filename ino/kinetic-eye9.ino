/*
 * PROJECT: Kinetic Eye - Spectral Fluorometer
 * FILE: KineticEye_Worker_V5.ino
 * DATE: April 17, 2026
...
#define UV_EMIT    14
#define BTN_START  12
#define SENSOR_PIN 13  // Single Analog Input for RGB & UV
...
void setup() {
  ...
  pinMode(UV_EMIT, OUTPUT);
  pinMode(BTN_START, INPUT_PULLUP);

  // Initialize Bluetooth
  SerialBT.begin("KineticEye_Worker"); 
  Serial.println(F("STATUS: Fluorometer Ready. Connect Termux and press Button."));
}

void runSpectralScan() {
  // Notify Phone to open the data "bucket"
  ...
  // Fire UV Emitter
  digitalWrite(UV_EMIT, HIGH);
  delay(100); // Wait for the "Glow" to stabilize

  // Take an averaged "Sane" reading of the visible fluorescence
  long fluorTotal = 0;
  int samples = 100;
  for(int i=0; i < samples; i++) {
    fluorTotal += analogRead(SENSOR_PIN);
    delay(1);
  }
  
  // Send the specific Fluorophore Signature
  SerialBT.print(F("FLUOR_SIG:"));
  SerialBT.println(fluorTotal / samples);
  
  // Signal phone to Beep, Vibrate, and save the file
  SerialBT.println(F("END_SCAN"));
  
  // Power down UV to save heat/battery
  digitalWrite(UV_EMIT, LOW);
}
